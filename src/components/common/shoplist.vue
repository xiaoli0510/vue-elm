<template>
  <div class="shoplist_container">
    <ul v-load-more="loadMore" v-if="state.shopListArr.length" type="1">
      <router-link
    ...
    <p v-if="state.touchend" class="empty_data">没有更多了</p>
    <aside class="return_top" @click="backTop" v-if="state.showBackStatus">
    ...
    <transition name="loading">
      <loading v-show="state.showLoading"></loading>
    </transition>
  </div>
</template>
...
export default {
  setup(props) {
    ...
    const { latitude, longitude } = storeToRefs(mainStore);
   ...
   function backTop() {
	...
	watch(()=>props.restaurantCategoryIds,
      (value)=>{
		   this.listenPropChange();
	  });

	  watch(()=>props.sortByType,
      (value)=>{
		  this.listenPropChange();
	  });

	  watch(()=>props.confirmSelect,
      (value)=>{
		   this.listenPropChange();
	  });

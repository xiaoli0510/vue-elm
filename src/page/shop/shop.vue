          ...
          (foodid) => {
            ...
            if (item.type == 1) {
              ...
              if (foodItem.num > 0) {
                ...
                state.cartFoodList[cartFoodNum].category_id =
...
<template>
  <div>
    <section v-if="!state.showLoading" class="shop_container">
      ...
      <header
        ...
        ref="shopheader"
        :style="{ zIndex: state.showActivities ? '14' : '10' }"
      >
        ...
        <section class="description_header">
          <router-link to="/shop/shopDetail" class="description_top">
            ...
            <section class="description_right">
              <h4 class="description_title ellipsis">
                {{ state.shopDetailData.name }}
              </h4>
              <p class="description_text">
                商家配送／{{
                  state.shopDetailData.order_lead_time
          ...
          <footer
            ...
            v-if="state.shopDetailData.activities.length"
          ...
          >
            <p class="ellipsis">
              <span
                ...
                :style="{
                  backgroundColor:
                    '#' + state.shopDetailData.activities[0].icon_color,
                  borderColor: '#' + shopDetailData.activities[0].icon_color,
                }"
                >{{ state.shopDetailData.activities[0].icon_name }}</span
              >
              <span
                >{{
                  state.shopDetailData.activities[0].description
                }}（APP专享）</span
              >
            </p>
            <p>{{ state.shopDetailData.activities.length }}个活动</p>
      ...
      <transition name="fade">
        <section class="activities_details" v-if="state.showActivities">
          ...
          <section class="activities_list">
            ...
            <ul>
              <li
                v-for="item in state.shopDetailData.activities"
                :key="item.id"
              >
                <span
          ...
          <svg
            ...
            @click.stop="state.showActivitiesFun"
          >
            <circle
              cx="30"
              cy="30"
      ...
      <section class="change_show_type" ref="chooseType">
        <div>
          <span
            :class="{ activity_show: state.changeShowType == 'food' }"
            @click="state.changeShowType = 'food'"
            >商品</span
          >
        </div>
        <div>
          <span
            :class="{ activity_show: state.changeShowType == 'rating' }"
            @click="state.changeShowType = 'rating'"
            >评价</span
          >
        </div>
      </section>
      <transition name="fade-choose">
        <section v-show="state.changeShowType == 'food'" class="food_container">
            ...
            <section class="menu_left" id="wrapper_menu" ref="wrapperMenu">
              <ul>
                <li
                  ...
                  :key="index"
                  class="menu_left_li"
                  :class="{ activity_menu: index == state.menuIndex }"
              ...
              <ul>
                <li v-for="(item, index) in state.menuList" :key="index">
                  <header class="menu_detail_header">
                    ...
                    ></span>
                    <p
                      class="description_tip"
                      v-if="index == state.TitleDetailIndex"
                    >
                      <span>{{ item.name }}</span>
                      {{ item.description }}
                    </p>
          ...
          <section class="buy_cart_container">
            <section @click="toggleCartList" class="cart_icon_num">
              <div
                ...
                :class="{
                  cart_icon_activity: state.totalPrice > 0,
                  move_in_cart: state.receiveInCart,
                }"
                ref="cartContainer"
              >
            ...
            >
              ...
              <router-link
                :to="{
                  path: '/confirmOrder',
                  query: { geohash: state.geohash, shopId: state.shopId },
                }"
                class="gotopay_button_style"
                v-else
                >去结算</router-link
          ...
          <transition name="toggle-cart">
            <section
              ...
              v-show="state.showCartList && state.cartFoodList.length"
            >
              <header>
                <h4>购物车</h4>
                <div @click="clearCart">
              ...
              <section class="cart_food_details" id="cartFood">
                <ul>
                  <li
                    v-for="(item, index) in state.cartFoodList"
                    :key="index"
                    class="cart_food_li"
                  >
      ...
      <transition name="fade-choose">
        <section
          ...
          id="ratingContainer"
          v-show="state.changeShowType == 'rating'"
        >
          <section type="2">
            <!-- <section v-load-more="state.loaderMoreRating" type="2"> -->
            <section>
              <header class="rating_header">
                <section class="rating_header_left">
                  <p>{{ state.shopDetailData.rating }}</p>
                  <p>综合评价</p>
                  <p>
                    高于周边商家{{
                      (state.ratingScoresData.compare_rating * 100).toFixed(1)
                    }}%
                  </p>
                </section>
                <section class="rating_header_right">
                  ...
                  <p>
                    ...
                    <span class="delivery_time"
                      >{{ state.shopDetailData.order_lead_time }}分钟</span
                    >
                  </p>
                </section>
              </header>
              <ul class="tag_list_ul">
                <li
                  v-for="(item, index) in state.ratingTagsList"
                  :key="index"
                  :class="{
                    unsatisfied: item.unsatisfied,
                    tagActivity: state.ratingTageIndex == index,
                  }"
              ...
              <ul class="rating_list_ul">
                <li
                  v-for="(item, index) in state.ratingList"
                  :key="index"
